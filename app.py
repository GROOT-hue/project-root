...
def summarize():
    ...
    if not text.strip() or len(sentences) < 2:
        return jsonify({'error': 'Please enter at least two sentences.', 'stats': stats}), 400
...
def debug_code():
    ...
    if not code.strip():
        return jsonify({'issues': ['Code is empty.']})
    ...
    try:
        ...
        Run(['temp_code.py', '--reports=n'], reporter=reporter, exit=False)
